...
const LeftH = styled('path')``;
const RightH = styled('path')(() => ({
  animation: `${keyframes`
  to {
...
const Logo = ({ isActive, ...props }: React.SVGProps<SVGSVGElement> & { isActive: boolean }) => {
  const { isDarkMode } = useDarkModeContext();
  const isWhite = isDarkMode ? true : !isActive;
  return (
    <LogoSvg xmlns="http://www.w3.org/2000/svg" width="1000" height="400" viewBox="0 0 1000 400" {...props}>
      ...
      <g>
        <RightH
          id="rightH"
          fill={isWhite ? 'var(--yellow-500)' : 'url(#end-gradient)'}
          data-name="}"
      ...
      <LeftH
        id="leftH"
        fill={isWhite ? 'var(--yellow-500)' : 'url(#start-gradient)'}
        data-name="{"
      ...
      <g>
        <CenterH
          fill={isWhite ? 'var(--yellow-500)' : 'var(--blue-500)'}
          id="centerH"
          data-name="-"
          d="M153,180H293v30H153V180Z"
        />
      </g>
      <Reminder
        fill={`var(${isWhite ? '--gray-050' : '--blue-600'})`}
        id="ikun"
